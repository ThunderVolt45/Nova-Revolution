...
void UNovaUnitInfoWidget::HandleSelectionChanged(const TArray<AActor*>& SelectedActors)
{
	...
	{
		CurrentObservedUnit->OnUnitAttributeChanged.RemoveDynamic(this, &ThisClass::OnUnitAttributeChanged);
		CurrentObservedUnit = nullptr;
	}
	if (CurrentObservedBase)
	{
		CurrentObservedBase->OnBaseAttributeChanged.RemoveDynamic(this, &ThisClass::OnBaseAttributeChanged);
		CurrentObservedBase = nullptr;
	}
	...
	if (SelectionCount == 1)
	{
		...
		if (ANovaUnit* Unit = Cast<ANovaUnit>(SelectedActors[0]))
		{
			...
			CurrentObservedUnit->OnUnitAttributeChanged.AddDynamic(this, &ThisClass::OnUnitAttributeChanged);

			UpdateUnitInfo(Unit);
		}
		// 기지
		else if (ANovaBase* Base = Cast<ANovaBase>(SelectedActors[0]))
		{
			if (WidgetSwitcher) WidgetSwitcher->SetActiveWidgetIndex(0);
			CurrentObservedBase = Base;
			CurrentObservedBase->OnBaseAttributeChanged.AddDynamic(this, &ThisClass::OnBaseAttributeChanged);
...
void UNovaUnitInfoWidget::OnUnitAttributeChanged(ANovaUnit* Unit)
{
...
void UNovaUnitInfoWidget::OnBaseAttributeChanged(ANovaBase* Base)
{
	// 감시 중인 기지와 일치하는지 확인 후 갱신
	if (Base == CurrentObservedBase)
	{
		UpdateBaseInfo(Base);
	}
}
...
void UNovaUnitInfoWidget::UpdateBaseInfo(ANovaBase* Base)
{
	if (!Base) return;
	...
	if (UnitNameText)
	{
		UnitNameText->SetText(FText::FromString(Base->GetBaseName()));
	}

	// AttributeSet을 통해 상세 정보 갱신
	if (const UNovaAttributeSet* AS = Base->GetAttributeSet())
	{
		const float CurHP = AS->GetHealth();
		const float MaxHP = AS->GetMaxHealth();
		const float HPPercent = (MaxHP > 0.f) ? (CurHP / MaxHP) : 0.f;
